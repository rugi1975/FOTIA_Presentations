...
\begin{figure}
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
                \includegraphics[width=\textwidth]{leaf.jpg}
                %\caption{FOTIA Column}
                %\label{fig:FOTIA Column}
        \end{subfigure}%
               
        \caption{LEAF}\label{fig:LEAF}
\end{figure}

\begin{itemize}

\item \textbf{Low Energy Accelerator Facility(LEAF)} is a Low energy(upto 50 keV ) high current( order of hundreds of nano ampere) \textbf{negative} ion accelerator.

\item Extensively used for molecular and irradiation experiments. IIT -B nano electronics group is a prominent user.

\item Used for technology testing.
